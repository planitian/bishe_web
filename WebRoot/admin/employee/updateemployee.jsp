...
console.log("进入");
 ...
 console.log((emp.length==0));
  ...
  if(emp!="<%=request.getParameter("employeeid")%>"){
  $.ajax({
     type:"POST",
 ...
 function tijiao(){
	...
	"
							...
							$.ajax({
								type : "POST",
								url : "ajax/updateemployee.action",
								async : false,
								data : dataa,
								dataType : "json",
								success : function(data) {
...
<body>
	...
	%>
	<div class="formbody" id="content">
		<form name="emplo" id="biaodan22" enctype="multipart/form-data">
			<ul class="forminfo">
				<li><label>医生编号</label><input name="employeeid" id="employeeid"
					type="text" class="dfinput" value="${param.employeeid }" /><i>不能超过30个字符</i>
				</li>
				<li><label>姓名</label><input name="name" id="name" type="text"
					class="dfinput" value="${param.name }" /><i></i></li>
				<li><label>密码</label><input name="password" id="password"
					type="text" class="dfinput" value="${param.password }" /><i>不能超过30个字符</i>
				</li>
				...
				<li><label>现在头像</label><img alt="现在头像"
					src="${param.imgurl }?t=<%=Math.random() %>" width="60px"
					height="60px"></li>
				<li><label style="width: 100%;display: block;">头像修改</label>
				...
				<li style="margin-top: 80px"><label>&nbsp;</label><input
					onclick="resetbiaodan()" type="button" class="btn" value="重置" /> <input
					type="button" class="btn" id="tt" value="提交" /></li>
			</ul>
